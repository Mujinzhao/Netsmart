...
import java.util.regex.Pattern;
	...
	public class RegexValidate {
			...
			public static final String SINGLEQUOTES="/\"/g";
		    //------------------常量定义
			...
			//^[+-](0.(\\d+)(\\.\\d{1,})?$)
			//^[-+]?[0-9]+(\.[0-9]+)?$
			public static final String	DOUBLE ="^[-+]?([0-9]+.(\\d+)$)";
...
//		 		Sy stem.out.println("pattern="+pattern);
		 	...
		 	public static  boolean isScientific(String str){
				if(StrisNull(str))
					return false; 
				return Regular(str,RegexValidate.SCIENTIFIC_A);
		 	}	
		 	/**
			 * @类功能说明：处理转义字符&和'针对ORACLE数据库
			 * @创建日期：2013-8-21
			 * @版本：V1.0
			*/
...
//				    String specialStr = "<div id=\"main_c\">.*?<a href=\"(.*?)\".*?<font color=red><B>.*?</font>(.*?)</a>";
